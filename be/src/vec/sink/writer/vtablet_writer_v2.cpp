...
Status VTabletWriterV2::_init(RuntimeState* state, RuntimeProfile* profile) {
    ...
    _mflush_lock_timer = ADD_CHILD_TIMER(_profile, "MFlushLockTime", "MFlushTime");
...
Status VTabletWriterV2::_write_memtable(std::shared_ptr<vectorized::Block> block, int64_t tablet_id,
    ...
    }
    {
        ...
        timers t {};
        RETURN_IF_ERROR(delta_writer->write(block.get(), rows.row_idxes, t));
        COUNTER_UPDATE(_init_timer, t.init_timer);
        COUNTER_UPDATE(_wait_timer, t.wait_timer);
        COUNTER_UPDATE(_write_timer, t.write_timer);
        COUNTER_UPDATE(_lock_timer, t.lock_timer);
        COUNTER_UPDATE(_mlock_timer, t.mlock_timer);
...
Status VTabletWriterV2::close(Status exec_status) {
    ...
    if (status.ok()) {
        ...
        // close DeltaWriters
        {
            ...
            COUNTER_UPDATE(_mflush_lock_timer, t.mflush_lock_timer);
